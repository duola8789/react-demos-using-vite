 ...
 (function() {
  ...
  var browsers = [{
      ...
      search: function(uastring) {
        return (
          contains$1(uastring, 'edge/') && contains$1(uastring, 'chrome') && contains$1(uastring, 'safari') && contains$1(uastring, 'applewebkit')
        );
      }
    },
    {
  ...
  var reqMessage = function(required, keys) {
    throw new Error(
      'All required keys (' + sort$1(required).join(', ') + ') were not specified. Specified keys were: ' + sort$1(keys).join(', ') + '.'
    );
  };
  ...
  var shouldStyleWithCss = function(editor) {
    return editor.getParam('table_style_by_css', true, 'boolean');
  };
      ...
      return last$1(cell)
        .bind(function(rightEdge) {
          ...
          return parent(rightEdge).map(function(parent) {
            return rightSiblingIsBlock === true || isListItem(parent) || isBr(rightEdge) || (isBlock$1(parent) && !eq$1(cell, parent)) ?
              [] :
  ...
  var insert$1 = function(editor, columns, rows, colHeaders, rowHeaders) {
    ...
    var attributes = __assign(getDefaultAttributes(editor), { border: '1' });
    var options = {
  ...
  var extractDataFromTableElement = function(editor, elm, hasAdvTableTab) {
    var getBorder = function(dom, elm) {
      ...
      var borderWidthTDTHOverallStyle = getTDTHOverallStyle(editor.dom, elm, 'border-width');
      var borderTable = dom.getAttrib(elm, 'border');
      var borderTDTHOverallStyle = getTDTHOverallStyle(editor.dom, elm, 'border');
      var border = borderWidthTDTHOverallStyle || borderTable || borderTDTHOverallStyle;
      var borderAll = +removePxSuffix(borderWidthTDTHOverallStyle) || +removePxSuffix(borderTable) || +removePxSuffix(borderTDTHOverallStyle);
      return {
        border: border,
        borderAll: !!borderAll
      }
  ...
  var applyDataToElement = function(editor, tableElm, data, oldData) {
    var dom = editor.dom;
    var attrs = {};
    var styles = {};
    attrs.class = data.class;
    styles.height = addPxSuffix(data.height);
    var newWidth = typeof data.width === 'undefined' ? oldData.width : ''
    if (dom.getAttrib(tableElm, 'width') && !shouldStyleWithCss(editor)) {
      attrs.width = removePxSuffix(newWidth);
    } else {
      styles.width = addPxSuffix(newWidth);
    }
    ...
    var borderTDTH = {};
    if (data.borderAll) {
      borderTDTH = { 'border-width': '1px', 'border-style': 'solid', 'border-color': '#ccc' }
    } else {
      borderTDTH = { 'border': 'none' }
      attrs.border = '0';
    }
    if (shouldStyleWithCss(editor) && tableElm.children) {
      for (var i = 0; i < tableElm.children.length; i++) {
        styleTDTH(dom, tableElm.children[i], __assign(borderTDTH, {
          padding: addPxSuffix(data.cellpadding)
        }));
        if (hasAdvancedTableTab(editor) && data.bordercolor) {
  ...
  var onSubmitTableForm = function(editor, tableElm, oldData, api) {
    ...
    var captionElm;
    var data = api.getData();
    var modifiedData = filter$1(oldData, function(value, key) {
    ...
    editor.undoManager.transact(function() {
      ...
      if (size(modifiedData) > 0) {
        applyDataToElement(editor, tableElm, data, oldData);
  ...
  var open$2 = function(editor, insertNewTable) {
    ...
    editor.windowManager.open({
      ...
      onChange: function(api) {
        var borderAll = api.getData().borderAll;
        if (borderAll) {
            api.enable('bordercolor');
            return;
        }
        api.disable('bordercolor');
      }
    });
  };
  ...
  var getSelectionTargets = function(editor, selections) {
    ...
    return {
      ...
      isDisabledForSelection: isDisabledForSelection,
      targets: function() {
        return targets.get();
      }
    };
  ...
  var addButtons = function(editor, selectionTargets, clipboard) {
    ...
    editor.ui.registry.addMenuButton('lxTable', {
  ...
  var addMenuItems = function(editor, selectionTargets, clipboard) {
    ...
    var row = {
      ...
      getSubmenuItems: function() {
        // return 'tableinsertrowbefore tableinsertrowafter tabledeleterow tablerowprops | tablecutrow tablecopyrow tablepasterowbefore tablepasterowafter'; 定制需求
        return 'tableinsertrowbefore tableinsertrowafter tabledeleterow';
      }
    };
    ...
    var column = {
      ...
      icon: 'table-column',
      getSubmenuItems: function() {
        // return 'tableinsertcolumnbefore tableinsertcolumnafter tabledeletecolumn | tablecutcolumn tablecopycolumn tablepastecolumnbefore tablepastecolumnafter'; 定制需求
    ...
    var cell = {
      ...
      icon: 'table',
      getSubmenuItems: function() {
        // return 'tablecellprops tablemergecells tablesplitcells'; 定制需求不要 tablecellprops
        return 'tablemergecells tablesplitcells';
      }
    };
    var contexttable = {
      type: 'nestedmenuitem',
    ...
    editor.ui.registry.addNestedMenuItem('contexttable', contexttable);
    editor.ui.registry.addContextMenu('table', {
      update: function() {
        selectionTargets.resetTargets();
        return selectionTargets.targets().fold(
          function() {
            return editor.selection.getContent() ? [''] : ['contexttable'];
          },
          function(targets) {
            var mergeDisabled = selectionTargets.isDisabledForSelection('mergeable');
            var splitDisabled = selectionTargets.isDisabledForSelection('unmergeable');
            ...
            if (name(targets.element) === 'caption') {
              ...
              var mergeSplit = '';
              if (!mergeDisabled) {
                mergeSplit = 'tablemergecells |';
              }
              if (!splitDisabled) {
                mergeSplit = 'tablesplitcells |';
              }
              return (
                mergeSplit +
                ' tableinsertcolumnbefore tableinsertcolumnafter tableinsertrowbefore tableinsertrowafter | tableprops | tabledeleterow tabledeletecolumn deletetable'
              );
            }
          }
        );
