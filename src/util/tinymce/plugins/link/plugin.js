...
(function() {
    ...
    var handleSubmit = function(editor, info) {
        return function (api) {
            ...
            if (!/(https?)|(file)|(ftp)|(mailto):\/?\/?\/?([\w.]+\/?)\S*/.test(url)) {
                url = 'http://' + url;
            }
            var getChangedValue = function (key) {
                if (key === 'text' && !data[key] && onlyText) {
                    data.text = data.url.value;
                }
                return Optional.from(data[key])
    ...
    var makeDialog = function(settings, onSubmit, editor) {
        ...
        //     name: 'url',
        //     type: 'urlinput',
        //     placeholder: '链接地址：例如https://office.163.com',
        //     filetype: 'file',
        //     label: ''
        // }];
        // var urlInput = settings.anchor.text.map(function() {
        //     return {
        //         name: 'url',
        //         type: 'urlinput',
        //         placeholder: '链接地址：例如https://office.163.com',
        ...
        return {
            ...
            onChange: function(api, _a) {
                var urlData = api.getData().url.value;
                if (urlData) {
                    api.enable('comfirm');
                    return;
                }
