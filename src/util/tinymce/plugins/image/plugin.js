...
(function() {
    ...
    var replaceImgUrlCallback = function (editor, params) {
        const getParams = function (key, url) {
            var temData = new URLSearchParams(url);
        ...
        const originFileUrlArr = [];
        const replaceFileUrlArr = [];
        params.forEach(function(params) {
            var originFileUrl = params.originFileUrl;
            var replaceFileUrl = params.replaceFileUrl;
            // originFileUrlArr.push(originFileUrl);
            var onOriginalUrl = 'part=' + (getParams('part', originFileUrl) || getParams('attachId', originFileUrl));
            originFileUrlArr.push(onOriginalUrl);
            replaceFileUrlArr.push(replaceFileUrl);
        });
        // var originFileUrl = params.originFileUrl;
        // var replaceFileUrl = params.replaceFileUrl;
        var images = editor.getBody().querySelectorAll('img');
        for (let i = 0; i < images.length; i++) {
            // let onIndex = originFileUrlArr.indexOf(images[i].getAttribute('src'));
            // if (onIndex !== -1) {
            //     images[i].setAttribute('src', replaceFileUrlArr[onIndex]);
            // }
            var onSrc = 'part=' + getParams('attachId', images[i].getAttribute('src'));
            let onIndex = originFileUrlArr.indexOf(onSrc);
            if (onIndex !== -1) {
                images[i].setAttribute('src', replaceFileUrlArr[onIndex]);
            }
        }
    ...
    var register = function(editor) {
        ...
        editor.on('replaceImgUrl', function(params) {
            replaceImgUrlCallback(editor, params);
